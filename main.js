...
(function() {
  ...
  /* Fix --vh para iOS Safari (100vh inclui barra de endereço) */
  function setVH() {
  ...
  /* Funciona em qualquer largura — sem bloqueio mobile */
  function onScroll() {
...
/* WHO FAN — Cards em leque por scroll */
(function() {
  const wrapper = document.getElementById('whoFanWrapper');
  const cards   = document.querySelectorAll('.who-fan-card');

  if (!wrapper || cards.length === 0) return;
  if (window.innerWidth <= 768) return; // mobile: layout estático
  ...
  function onScroll() {
    ...
    // ratio: 0 = início, 1 = fim do wrapper
    const ratio = Math.max(0, Math.min(1, scrolled / total));

    // Leque abre a partir de ratio 0.1
    const fanRatio = Math.max(0, Math.min(1, (ratio - 0.1) / 0.6));

    cards.forEach((card) => {
      const rotate = parseFloat(card.style.getPropertyValue('--card-rotate')) || 0;
      const tx     = parseFloat(card.style.getPropertyValue('--card-tx')) || 0;
      const ty     = parseFloat(card.style.getPropertyValue('--card-ty')) || 0;

      const currentRotate = rotate * fanRatio;
      const currentTx     = tx * fanRatio;
      const currentTy     = ty * fanRatio;

      card.style.transform = `rotate(${currentRotate}deg) translate(${currentTx}px, ${currentTy}px)`;
    });
  }
  ...
  cards.forEach(card => {
    card.addEventListener('mouseenter', () => {
      ...
      card.style.transform = currentTransform + ' translateY(-12px) scale(1.02)';
    });
    card.addEventListener('mouseleave', () => {
      card.style.zIndex = '';
      onScroll(); // restaura posição do scroll
    });
  });
