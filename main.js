...
/* ANIMAÇÃO SCROLL — SEÇÃO DE PASSOS (desktop + mobile) */
(function() {
  ...
  /* Corrige --vh para iOS Safari (barra de endereço inclusa no 100vh) */
  function setVH() {
  ...
  /* Ativa o passo correto com animação */
  ...
  /* Barra de progresso: horizontal no mobile, vertical no desktop */
  function updateProgress(ratio) {
    if (!progressFill) return;
    const pct = ((ratio * TOTAL_STEPS) % 1) * 100;
  ...
  /* Scroll handler — funciona em desktop e mobile */
  function onScroll() {
    ...
    const active = Math.min(Math.floor(ratio * TOTAL_STEPS), TOTAL_STEPS - 1);

    activateStep(active);
    updateProgress(ratio);
  }

  /* Inicializa com o passo 1 ativo */
